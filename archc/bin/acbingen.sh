...
version="\
ArchC binutils generator script version 2.0"
...
if [ -z "$BINUTILS_PATH" ]; then
  BINUTILS_PATH=`grep BINUTILS_PATH /home/gustavomart/mc723/archc/etc/archc.conf | cut -d = -f2`
...
mkdir -p $TEMP_DIR
[ $? -ne 0 ] && exit $?
cp -rf /home/gustavomart/mc723/archc/share/archc/binutils ${TEMP_DIR}/


# generate the files in the binutils tree
echo "Generating machine dependent code..."
/home/gustavomart/mc723/archc/bin/bmdsfg $MODEL_FILE -a${ARCH_NAME}
[ $? -ne 0 ] && exit $?
...
if [ -z "$GDB_PATH" ]; then
  GDB_PATH=`grep GDB_PATH /home/gustavomart/mc723/archc/etc/archc.conf | cut -d = -f2`
...
mkdir -p $TEMP_DIR
[ $? -ne 0 ] && exit $?
cp -rf /home/gustavomart/mc723/archc/share/archc/gdb ${TEMP_DIR}/
